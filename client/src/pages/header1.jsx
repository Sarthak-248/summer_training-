...
const PatientNavbar = () => {
  ...
  useEffect(() => {
    ...
    socket.on('connect', () => {
      ...
      if (patientId) socket.emit('registerPatient', patientId);
    });
    ...
    socket.on('newDoctorListing', (data) => {
      ...
      setNotifications(prev => {
         const exists = prev.some(n => n.message === newNotification.message && Math.abs(new Date(n.time) - new Date()) < 5000);
    ...
    <nav className="h-16 w-full fixed top-0 z-50 bg-gradient-to-r from-slate-900 via-purple-950 to-slate-900 border-b border-white/10 shadow-lg backdrop-blur-md">
      ...
      <div className="h-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
        ...
        <div className="hidden lg:flex items-center gap-1">
          ...
          <NavLink to="/patient/appointments/upcoming">My Appointments</NavLink>
          <NavLink to="/patient/favorites">Favorites</NavLink>
        ...
        <div className="hidden lg:flex items-center gap-4">
          ...
          <Link
            ...
            className="px-5 py-2 bg-gradient-to-r from-pink-600 to-rose-600 hover:from-pink-500 hover:to-rose-500 text-white text-sm font-semibold rounded-lg shadow-lg hover:shadow-pink-500/25 transition-all duration-200"
          >
            Book Appointment
          </Link>
      ...
      {menuOpen && (
        <div className="lg:hidden absolute top-16 left-0 w-full bg-slate-900/95 backdrop-blur-xl border-b border-white/10 shadow-xl p-4 flex flex-col gap-2">
          ...
          <Link
            to="/patient/appointments"
            className="p-3 rounded-lg bg-blue-600 text-center text-white font-bold shadow-lg mt-2"
            onClick={() => setMenuOpen(false)}
          >
            Book Appointment
          </Link>
